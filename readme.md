...
| 15 - LocalStorage                                  | Aug 31, 2020 | CSS: Custom checkbox.<br>JS: Local storage, event delegation ("responsible parents, negligent children" - you can add an event listener to a parent element and then find out which child triggered it). Challenge: add uncheck/check all and clear list buttons. I have a solution but it's not working as smoothly as the addition. (`location.reload()` seems to be a more visible refresh of the page than when adding a list item, don't know why.) |
